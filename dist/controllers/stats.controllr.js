...
export const getDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (nodeCache.has("admin-stats"))
        ...
        const latestTransactionPromise = Order.find({})
            ...
            .limit(4);
        const [thisMonthProducts, thisMonthUsers, thisMonthOrders, lastMonthProducts, lastMonthOrders, lastMonthUsers, productsCount, usersCount, allOrders, lastSixMonthOrders, categories, femaleUsersCount, latestTransactions,] = await Promise.all([
        ...
        nodeCache.set("adimn-stats", JSON.stringify(stats));
    }
    res.status(200).json({
        success: true,
        stats,
...
export const getPieCharts = TryCatch(async (req, res, next) => {
    ...
    if (nodeCache.has("admin-pie-charts"))
        charts = JSON.parse(nodeCache.get("admin-pi-charts"));
    else {
